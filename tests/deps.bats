...
@test "load_module_deps skips blank lines and comments" {
  create_module "mymod"
...
@test "load_module_deps strips inline comments" {
  create_module "mymod"
...
@test "load_module_deps warns on unknown directives" {
  create_module "mymod"
  printf '%s\n' 'after: foo' 'bogus: something' >"$TEST_CONFIG/modules/mymod/deps"

  run load_module_deps "$TEST_CONFIG" "mymod"
  [ "$status" -eq 0 ]
