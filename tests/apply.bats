...
@test "apply: generates rc file from module bash configs" {
  ...
  grep -q 'export ALPHA="yes"' "$NOMADIC_DIR/config.bash"
  grep -q 'export BETA="yes"' "$NOMADIC_DIR/config.bash"
  grep -q 'Generated by nomadic' "$NOMADIC_DIR/config.bash"
}
...
@test "apply: filters modules by OS" {
  ...
  [ -f "$NOMADIC_DIR/config.bash" ]
  grep -q 'export UNI="yes"' "$NOMADIC_DIR/config.bash"
...
@test "apply: preserves dependency order in rc file" {
  ...
  first_line="$(grep -n 'FIRST' "$NOMADIC_DIR/config.bash" | head -1 | cut -d: -f1)"
  second_line="$(grep -n 'SECOND' "$NOMADIC_DIR/config.bash" | head -1 | cut -d: -f1)"
  third_line="$(grep -n 'THIRD' "$NOMADIC_DIR/config.bash" | head -1 | cut -d: -f1)"
...
@test "apply: remembers config path" {
  ...
  grep -q "$TEST_CONFIG" "$NOMADIC_DIR/state/config-path"
}
