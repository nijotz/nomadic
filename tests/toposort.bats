...
@test "detects simple cycle" {
  ...
  run bash -c "source '$NOMADIC_ROOT/nomadic' && load_modules '$TEST_CONFIG' foo bar && toposort"
  [ "$status" -eq 1 ]
...
@test "detects three-node cycle" {
  ...
  run bash -c "source '$NOMADIC_ROOT/nomadic' && load_modules '$TEST_CONFIG' A B C && toposort"
  [ "$status" -eq 1 ]
  echo "$output" | grep -q 'cycle'
}
