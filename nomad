...
cmd_apply() {
  ...
  # Symlink nomad itself into ~/.local/bin so it's on PATH via the generated rc
  local nomad_bin="$HOME/.local/bin"
  local nomad_self
  nomad_self="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$(basename "${BASH_SOURCE[0]}")"
  mkdir -p "$nomad_bin"
  if [[ -L "$nomad_bin/nomad" ]] && [[ "$(readlink "$nomad_bin/nomad")" == "$nomad_self" ]]; then
    log "nomad already linked in $nomad_bin"
  elif [[ -e "$nomad_bin/nomad" ]]; then
    warn "$nomad_bin/nomad already exists, skipping self-link"
  else
    ln -s "$nomad_self" "$nomad_bin/nomad"
    log "Linked nomad -> $nomad_bin/nomad"
  fi
